...
- [sketch application](apps.md#sketch) — Pointer-driven graphical paint application. Its layout follows classic desktop paint programs: Save, Open, Import, and the sidebar controls share one aligned, equal-sized button grid; color and pattern choices are in the bottom bar. Sketch uses a white four-shade canvas and stores finished documents as interoperable 8-bit grayscale PNG files.
