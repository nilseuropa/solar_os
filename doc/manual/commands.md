...
| `say` | `say [-v <0..100>] [--volume <0..100>] [--drop-if-busy] ([--force] --file <path> \| [--] <text...>)` | Queue text for offline speech, or read a plain UTF-8 text file aloud with progress. Files larger than 64 KiB require `--force`. File mode completes paths with Tab and runs in the foreground; press Esc or Ctrl+C to stop. Start `speechd` with a PicoTTS voice directory first. |
