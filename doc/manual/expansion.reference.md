...
| `mgc3130` | MGC3130/Skywriter 3D gesture sensor | `i2c=<bus> addr=0x42|0x43 transfer=<pin> reset=<pin> rotation=<0..3>`; optional `airwheel=0|1` | Uses the open-drain transfer handshake and publishes normalized X/Y/Z axes, recognized gesture/tap/AirWheel events, and display-scaled absolute pointer motion. Electrode touch drives the primary pointer button. AirWheel defaults on; set it to 0 to make the mutually exclusive circle gestures available. |
