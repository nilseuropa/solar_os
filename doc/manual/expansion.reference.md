...
| TTGO VGA32 v1.4 | GPIO2, GPIO12-GPIO14, GPIO34, GPIO39 | GPIO34, GPIO39 | GPIO34, GPIO39 | GPIO34/GPIO39 are input-only. GPIO2/GPIO12-GPIO14 belong to the onboard v1.4 microSD interface; VGA and PS/2 signals are fixed board resources. |
...
| TTGO VGA32 v1.4 | `spi0`: SCK GPIO14, MISO GPIO2, MOSI GPIO12, CS GPIO13; `uart0`: TX GPIO1, RX GPIO3; `ps2kbd0`: clock GPIO33, data GPIO32 | None | Start the built-in keyboard with `job start ps2-keyboard ps2kbd0`. I2S1 and the six RGB plus two sync pins are permanently reserved for VGA DMA scanout. |
