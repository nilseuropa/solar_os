...
| `gt911` | GT911 capacitive touch controller | `i2c=<bus> addr=0x5d irq=<pin> rotation=<0..3>` | Polls absolute pointer events into the shared input service. Some panels strap the controller at I2C address `0x14`; bind the documented address explicitly so resource ownership matches the address in use. |
