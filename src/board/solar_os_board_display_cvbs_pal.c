...
static cvbs_pal_t cvbs_display;

static void display_bind_cvbs(solar_os_board_display_t *display)
{
    display->driver = &cvbs_display;
    display->driver_name = "cvbs-pal";
    display->u8g2 = cvbs_pal_get_u8g2(&cvbs_display);
...
esp_err_t solar_os_board_display_init(solar_os_board_display_t *display)
{
    ...
    const esp_err_t err = cvbs_pal_init(&cvbs_display);
    if (err != ESP_OK) {
        return err;
    }
    display_bind_cvbs(display);
    return ESP_OK;
}
...
esp_err_t solar_os_board_display_resume(solar_os_board_display_t *display)
{
    ...
    const esp_err_t err = cvbs_pal_resume((cvbs_pal_t *)display->driver);
    if (err != ESP_OK) {
        display->ready = false;
        return err;
    }
...
{
    if (display != NULL && display->driver != NULL) {
        cvbs_pal_deinit((cvbs_pal_t *)display->driver);
        memset(display, 0, sizeof(*display));
    }
}
