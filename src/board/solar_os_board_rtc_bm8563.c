...
static void rtc_datetime_from_driver(solar_os_board_rtc_datetime_t *out,
                                     const rtc_bm8563_datetime_t *in)
{
    out->year = in->year;
    out->month = in->month;
    out->day = in->day;
...
static void rtc_datetime_to_driver(rtc_bm8563_datetime_t *out,
...
esp_err_t solar_os_board_rtc_get_datetime(solar_os_board_rtc_datetime_t *datetime)
{
    ...
    rtc_bm8563_datetime_t driver_datetime;
    const esp_err_t err = rtc_bm8563_get_datetime(&driver_datetime);
    if (err != ESP_OK) {
        return err;
    }
...
esp_err_t solar_os_board_rtc_set_datetime(const solar_os_board_rtc_datetime_t *datetime)
{
    ...
    return rtc_bm8563_set_datetime(&driver_datetime);
}
