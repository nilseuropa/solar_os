...
static EXT_RAM_BSS_ATTR StaticSemaphore_t gnss_mutex_storage;
...
esp_err_t solar_os_gnss_set_power(const char *name, bool enabled)
{
    ...
    for (size_t i = 0; i < GNSS_DEVICE_MAX; i++) {
        if (gnss_devices[i].active && strcmp(gnss_devices[i].info.name, name) == 0) {
            if (!gnss_devices[i].info.power_control) {
                xSemaphoreGive(gnss_mutex);
    ...
    if (gnss_devices[index].active &&
        ...
        if (ret == ESP_OK) {
            gnss_devices[index].info.powered = enabled;
        }
    }
    xSemaphoreGive(gnss_mutex);
    return ret;
                                 ...
                                 solar_os_gnss_fix_t *fix)
{
    ...
    for (size_t i = 0; i < GNSS_DEVICE_MAX; i++) {
        if (gnss_devices[i].active && strcmp(gnss_devices[i].info.name, name) == 0) {
            if (!gnss_devices[i].info.powered) {
                xSemaphoreGive(gnss_mutex);
                return ESP_ERR_INVALID_STATE;
            }
            gnss_devices[i].refs++;
