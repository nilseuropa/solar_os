#include "solar_os_nfc.h"

#include <string.h>

#include "freertos/FreeRTOS.h"
#include "freertos/semphr.h"

#define NFC_DEVICE_MAX 4U

typedef struct {
    bool active;
    solar_os_nfc_info_t info;
    const solar_os_nfc_ops_t *ops;
    void *ctx;
} nfc_device_t;

static SemaphoreHandle_t nfc_mutex;
static StaticSemaphore_t nfc_mutex_storage;
static nfc_device_t nfc_devices[NFC_DEVICE_MAX];

static esp_err_t ensure_mutex(void)
{
    if (nfc_mutex == NULL) {
        nfc_mutex = xSemaphoreCreateMutexStatic(&nfc_mutex_storage);
    }
    return nfc_mutex != NULL ? ESP_OK : ESP_ERR_NO_MEM;
}
static bool name_valid(const char *name)
{
    return name != NULL && name[0] != '\0' &&
        strnlen(name, SOLAR_OS_NFC_NAME_MAX) < SOLAR_OS_NFC_NAME_MAX;
}

esp_err_t solar_os_nfc_register(const solar_os_nfc_registration_t *registration)
{
    if (registration == NULL || !name_valid(registration->name) ||
        registration->driver == NULL || registration->ops == NULL ||
        registration->ops->scan == NULL ||
        strnlen(registration->driver, SOLAR_OS_NFC_DRIVER_MAX) >=
            SOLAR_OS_NFC_DRIVER_MAX) {
        return ESP_ERR_INVALID_ARG;
    }
    if (ensure_mutex() != ESP_OK) {
        return ESP_ERR_NO_MEM;
    }
    xSemaphoreTake(nfc_mutex, portMAX_DELAY);
    nfc_device_t *free_device = NULL;
    for (size_t i = 0; i < NFC_DEVICE_MAX; i++) {
        if (nfc_devices[i].active &&
            strcmp(nfc_devices[i].info.name, registration->name) == 0) {
            xSemaphoreGive(nfc_mutex);
            return ESP_ERR_INVALID_STATE;
        }
        if (!nfc_devices[i].active && free_device == NULL) {
            free_device = &nfc_devices[i];
        }
    }
    if (free_device == NULL) {
        xSemaphoreGive(nfc_mutex);
        return ESP_ERR_NO_MEM;
    }
    memset(free_device, 0, sizeof(*free_device));
    free_device->active = true;
    strlcpy(free_device->info.name, registration->name, sizeof(free_device->info.name));
    strlcpy(free_device->info.driver, registration->driver, sizeof(free_device->info.driver));
    free_device->ops = registration->ops;
    free_device->ctx = registration->ctx;
    xSemaphoreGive(nfc_mutex);
    return ESP_OK;
}

esp_err_t solar_os_nfc_unregister(const char *name)
{
    if (!name_valid(name) || ensure_mutex() != ESP_OK) {
        return ESP_ERR_INVALID_ARG;
    }
    xSemaphoreTake(nfc_mutex, portMAX_DELAY);
    for (size_t i = 0; i < NFC_DEVICE_MAX; i++) {
        if (nfc_devices[i].active && strcmp(nfc_devices[i].info.name, name) == 0) {
            memset(&nfc_devices[i], 0, sizeof(nfc_devices[i]));
            xSemaphoreGive(nfc_mutex);
            return ESP_OK;
        }
    }
    xSemaphoreGive(nfc_mutex);
    return ESP_ERR_NOT_FOUND;
}

size_t solar_os_nfc_count(void)
{
    if (ensure_mutex() != ESP_OK) {
        return 0U;
    }
    size_t count = 0U;
    xSemaphoreTake(nfc_mutex, portMAX_DELAY);
    for (size_t i = 0; i < NFC_DEVICE_MAX; i++) {
        count += nfc_devices[i].active ? 1U : 0U;
    }
    xSemaphoreGive(nfc_mutex);
    return count;
}

bool solar_os_nfc_get(size_t index, solar_os_nfc_info_t *info)
{
    if (info == NULL || ensure_mutex() != ESP_OK) {
        return false;
    }
    size_t current = 0U;
    xSemaphoreTake(nfc_mutex, portMAX_DELAY);
    for (size_t i = 0; i < NFC_DEVICE_MAX; i++) {
        if (!nfc_devices[i].active) {
            continue;
        }
        if (current++ == index) {
            *info = nfc_devices[i].info;
            xSemaphoreGive(nfc_mutex);
            return true;
        }
    }
    xSemaphoreGive(nfc_mutex);
    return false;
}

esp_err_t solar_os_nfc_scan(const char *name,
                            uint32_t timeout_ms,
                            solar_os_nfc_tag_t *tag)
{
    if (!name_valid(name) || timeout_ms == 0U || tag == NULL || ensure_mutex() != ESP_OK) {
        return ESP_ERR_INVALID_ARG;
    }
    xSemaphoreTake(nfc_mutex, portMAX_DELAY);
    for (size_t i = 0; i < NFC_DEVICE_MAX; i++) {
        if (nfc_devices[i].active && strcmp(nfc_devices[i].info.name, name) == 0) {
            const esp_err_t ret = nfc_devices[i].ops->scan(
                nfc_devices[i].ctx, timeout_ms, tag);
            xSemaphoreGive(nfc_mutex);
            return ret;
        }
    }
    xSemaphoreGive(nfc_mutex);
    return ESP_ERR_NOT_FOUND;
}
